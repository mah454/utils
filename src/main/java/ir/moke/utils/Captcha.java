...
public class Captcha {
    ...
    static {
        try {
            File file = new File("/.RobotoSerif.ttf");
            if (file.exists()) {
                font = Font.createFont(Font.PLAIN, file).deriveFont(30f);
            } else {
...
//            g.setColor( new Color(random.nextInt(255), random.nextInt(255), random.nextInt(255)));
            ...
            g.drawString(new String(new byte[]{bytes[i]}), start + (i * 15), (int) (Math.random() * 20 + 20));
        }
        g.setColor(Color.white);
        for (int i = 0; i < 30; i++) {
